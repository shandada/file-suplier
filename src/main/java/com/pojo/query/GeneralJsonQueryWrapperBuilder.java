 ...
 *
 * 用于根据《通用HTTP接口设计》文档中的【一、实体CRUD操作的接口设计】，
 * 生成相应实体的 {@link QueryWrapper} 查询对象。
 *
 * 调用方法：
 * 1.以想要查询的实体类为泛型参数，实例化一个此类的对象（Builder对象）；
 * 2.使用本库中已实现的Query类：{@link }
 * 作为参数，调用Builder对象的 {@link #()}
 * 方法，query参数通常是在Controller中作为参数从前端传入的；
...
public class GeneralJsonQueryWrapperBuilder<EntityType> {
     ...
     */
    public QueryWrapper<EntityType> build(GeneralJsonEntityQuery query) throws ServiceException {
